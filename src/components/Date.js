...
const {setEndDate,setStarDate,startHour,endHour,starDate,endDate, setUrl,setUrlDistrito,tableHead,categoria,setUrlMapa} = useContext(Context)
    ...
    const mostrar=(date)=>{
        ...
        if(tipo==='incio'){
             ...
             if(tableHead==='categoria'){
                ...
                setStarDate((startDate)=>startDate=refd.current.value);
                 let inicio=refd.current.value;
                 let newUrl =`http://172.24.99.155:8000/api/map/?start=${inicio}T${startHour}:00:00.00Z&end=${endDate}T${endHour}:00:00.00Z`;
            ...
            if(tableHead==='categoria'){
                ...
                setEndDate((startDate)=>startDate=refd.current.value)
                let fin=refd.current.value
                let newUrl =`http://172.24.99.155:8000/api/map/?start=${starDate}T${startHour}:00:00.00Z&end=${fin}T${endHour}:00:00.00Z`;
                setUrlMapa(nUrl=>nUrl=newUrl)
    ...
    return (
        <>
          <input type="date" className="date" id="fecha" ref={refd} value={tipo==='incio'?starDate:endDate} onChange={mostrar} min="2003-01-01" max="2021-12-31"/>
        </>
    )
}
